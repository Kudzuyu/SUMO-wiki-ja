...
TraCI は{{SUMO_CMD}}[SUMO-link]へのアクセスのために、TCP ベースのクライアント/サーバー構造を使用します。
これによって{{SUMO_CMD}}[SUMO-link]はサーバーとして振舞い、コマンドラインオプション **`--repote-port`{{DT_INT}}(datatype-link)**をうけとってリスニングポートの番号とします。


**--remote-port {{DT_INT}}**オプションとともに開始すると、{{SUMO_CMD}}[SUMO-link]はシミュレーションのみを用意し、外部アプリケーションが接続してコントロールを引き受けるまで待機します。
**--end [`<TIME>`]***オプションは{{SUMO_CMD}}[SUMO-link]が TraCI サーバーとして動いている場合無視され、クライアントがシミュレーション終了を通知するまで動きつづけることに注意してください。

[SUMO-GUI]をサーバーとして使う場合、シミュレーションは[開始ボタン**を押して始めるか、**--start**オプションで TraCI コマンド実行前に開始しておく必要があります。

### 複数クライアント

シミュレーションに接続できるクライアントの数はオプション**--num-clients**{{DT_INT}}(datatype-link)**で設定できます(デフォルト: 1)。
...
以下の API において、ID は{{SUMO_CMD}}の入力ファイルにおいて定義された ID と同じものです。
...
TraCI を使うと、{{SUMO_CMD}}の**--end**オプションは無視されます。
...
* C++: [C++ TraCIAPI] はクライアントライブラリであり、{{SUMO_CMD}}のソースツリーの一部になっています(ほぼ全ての API をカバーしています)。
* C++: [Veins project](http://veins.car2x.org/) は{{SUMO_CMD}}と[OMNET++](https://omnetpp.org/)のミドルウェアを提供します。インフラの一部として TraCI API 用の C++クライアントライブラリが提供されています(API の完全性は Python クライアントに比べてわずかに劣ります)。
...
* Java: [Traas] は {{SUMO_CMD}}のソースツリーの一部としてクライアントライブラリを提供します(ほぼ全ての API をカバーしています)。
...
TraCI を用いることで、{{SUMO_CMD}} を通信ネットワークシミュレーションと組みあわせ、[車車間通信] のシミュレーションを実現できます。
詳細は [トピック/V2X] で確認できます。

### その他

* {{SUMO_CMD}} の TraCI サーバーは基本ディストリビューションの一部です。ソースコードは`src/traci-server`フォルダにあります。
...
第一世代は主に{{SUMO_CMD}}で使われている string ベースの ID と外部への表現として使われる int ベースの表現をマッピングします。
このマッピングは内部的に行なわれます(**TraCI**内で)。

現在の第二世代は{{SUMO_CMD}}が読みこむものと同じ文字列 ID を使用します。
もし第一世代の API を使うことを余儀なくされる場合 (例えば TraNS を使いたい場合)、バージョン 0.12.23 までの{{SUMO_CMD}}だけが使えます。
